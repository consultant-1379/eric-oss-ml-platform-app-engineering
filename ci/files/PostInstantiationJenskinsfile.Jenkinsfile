...
pipeline {
    ...
    stages {
        ...
        stage('Post Instantiation Testing') {
            steps {
                withCredentials([file(credentialsId: env.KUBE_CREDENTIALS, variable: 'KUBECONFIG'),
                    ...
                    usernamePassword(credentialsId: 'bragi_idun_user', passwordVariable: 'RAPP_PASSWORD', usernameVariable: 'RAPP_USERNAME')]) {
                        echo "Running Post Instantiation Tests"
                        //Generate data to the input topic to trigger the rApp 
                        //Check the produced result on output Kafka topic  
